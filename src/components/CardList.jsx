      ...
      <div className="bg-[#181e24] w-full min-h-screen mt-24 text-white flex flex-col lg:flex-row">
        <div className="w-full flex flex-col items-center justify-start lg:w-2/12 lg:mr-3 mt-2">
          ...
          <select className="bg-[#2a323c] rounded-md mx-8 my-3 px-2 pb-2 text-center " value={categoryName} onChange={(e) => { setcetagoryName(e.target.value); setisDisable(false); setbrandname(""); }} defaultValue="" >
            ...
            {category.map((name, index) => (<option key={index} value={name} className="text-left"> {name}  </option>))}
          </select>
          {!isDisable && (<select className="bg-[#2a323c] rounded-md mx-8 my-3 px-1 pb-1 text-center " value={brandname} onChange={(e) => { setbrandname(e.target.value); }} defaultValue="">
            <option value="" disabled>  By Brand </option>
            {Brands?.map((name, index) => (<option key={index} value={name} className="text-left"> {name} </option>))} </select>)}
        </div>
        <div className="w-full  mt-6 lg:mt-0">
          <div className="items-center ">
            ...
            <div className="flex flex-wrap justify-evenly lg:justify-start my-4">
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setcetagoryName(""); setbrandname(""); }}>  All</button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setisDisable(false); setcetagoryName("Restaurants"); }}>  Food</button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setcetagoryName("Electronics"); }}>  Electronics</button><button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setcetagoryName("Clothing"); }}>  Clothing</button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setcetagoryName("Travel"); }}> Travel</button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setcetagoryName("Subscription"); }}>Subcription</button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setbrandname("Samsung"); setcetagoryName(""); }}> Samsung </button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setbrandname("Hotstar"); setcetagoryName(""); }}> Hotstar </button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setbrandname("KFC"); setcetagoryName(""); }}>KFC </button>
              <button className="bg-[#2a323c] px-4 m-1 rounded-md" onClick={(e) => { setbrandname("DMart"); setcetagoryName(""); }} >DMart</button>
            </div>
          </div>
